...
export default function ChallengesPage() {
  ...
  const [diplomaPhoto, setDiplomaPhoto] = useState<string | null>(null); // Stato per la foto di gruppo
  ...
  // NUOVA FUNZIONE: Caricamento Foto di Gruppo per il Diploma
  async function handleDiplomaPhotoUpload(e: any) {
    const file = e.target.files[0];
    if (!file) return;

    const fileExt = file.name.split('.').pop();
    const fileName = `diploma-group-photo-${Date.now()}.${fileExt}`;

    const { error: uploadError } = await supabase.storage.from('media').upload(fileName, file);
    if (uploadError) { alert("Errore caricamento foto"); return; }

    const { data: { publicUrl } } = supabase.storage.from('media').getPublicUrl(fileName);
    
    setDiplomaPhoto(publicUrl);
    await supabase.from("Settings").update({ diploma_photo_url: publicUrl }).eq('id', 1);
    alert("Foto di gruppo salvata!");
  }
  ...
  const downloadPhoto = async (url: string, filename: string) => {
    ...
    } catch (err) { alert("Tieni premuto sul file per salvarlo."); }
  };
  ...
  async function handleUpload(e: any, challengeId: number) {
    ...
    const { data: { publicUrl } } = supabase.storage.from('media').getPublicUrl(fileName);
    await supabase.from("Challenges").update({ media_url: publicUrl, is_completed: true, caption: caption, bonus_achieved: isBonusSelected }).eq('id', challengeId);
  ...
  async function handleDelete(challengeId: number, mediaUrl: string) {
    if (!confirm("Eliminare prova?")) return;
    const fileName = mediaUrl.split('/').pop();
    if (fileName) await supabase.storage.from('media').remove([fileName]);
    await supabase.from("Challenges").update({ media_url: null, is_completed: false, caption: null, bonus_achieved: false }).eq('id', challengeId);
    setSelectedChallenge(null); fetchData();
  }

  const currentPoints = challenges.filter(c => c.is_completed).reduce((sum, c) => sum + (c.points || 0) + (c.bonus_achieved ? (c.bonus_points || 0) : 0), 0);
  const targetReached = currentPoints >= targetPoints;
  ...
  return (
    <main className="min-h-screen bg-[#0f0214] text-white p-4 md:p-8 pb-20 print:bg-white print:text-black print:p-0">
      ...
      <style jsx global>{`
        @media print {
          ...
          .printable-diploma { 
            ...
            align-items: center; border: 20px double #a855f7; padding: 40px; background: white;
          }
          @page { margin: 0; size: auto; }
        }
      `}</style>

      {/* SEZIONE DIPLOMA AGGIORNATA */}
      <div className={`printable-diploma hidden ${targetReached ? 'print:flex' : ''} flex-col items-center justify-center text-center bg-white text-black`}>
        <h1 className="text-5xl font-black uppercase tracking-tighter mb-2 text-purple-900" style={{ fontFamily: 'Montserrat, sans-serif' }}>Diploma di Idoneità</h1>
        <p className="text-xl font-bold uppercase tracking-[0.4em] mb-10 text-purple-600 border-b-2 border-purple-100 pb-2">Al Matrimonio</p>
        ...
        <p className="max-w-2xl text-lg leading-relaxed mb-8 px-10 text-center">
          Ha superato con gloria tutte le prove, totalizzando <span className="font-black text-purple-800">{currentPoints} Punti</span> e dimostrando di essere pronto per l'altare.
        </p>

        {/* SPAZIO FOTO DI GRUPPO SUL DIPLOMA */}
        {diplomaPhoto ? (
            <div className="mb-10 w-full max-w-lg">
                <img src={diplomaPhoto} className="w-full h-64 object-cover rounded-3xl border-8 border-purple-50 shadow-2xl" alt="Foto di gruppo" />
                <p className="text-[10px] mt-2 font-bold uppercase text-purple-400">I Testimoni del Misfatto</p>
            </div>
        ) : (
            <div className="mb-10 h-40 flex items-center justify-center border-4 border-dashed border-purple-100 rounded-3xl w-full max-w-sm bg-gray-50">
                <p className="text-gray-300 italic">Spazio per la foto di gruppo</p>
            </div>
        )}

        <div className="flex justify-between w-full max-w-2xl mt-4 border-t-2 border-purple-100 pt-6">
            <div className="text-left"><p className="font-bold uppercase text-[10px] text-purple-400">Data</p><p className="font-bold">{new Date().toLocaleDateString('it-IT')}</p></div>
            <div className="text-right"><p className="font-bold uppercase text-[10px] text-purple-400">La Commissione</p><p className="text-3xl text-purple-900" style={{ fontFamily: "'Dancing Script', cursive" }}>Gli Amici</p></div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto print:hidden">
        ...
        <div className="bg-white/5 p-6 rounded-[2.5rem] border border-white/20 mb-8 sticky top-4 z-30 backdrop-blur-3xl shadow-2xl">
            ...
            <div className="mb-6 p-6 bg-black/60 rounded-[1.5rem] border border-fuchsia-500/30 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="text-center">
                        ...
                        <input type="range" min="100" max="2000" step="50" value={targetPoints} onChange={(e) => updateTargetPoints(parseInt(e.target.value))} className="w-full accent-fuchsia-500" />
                        <p className="font-bold text-yellow-400 mt-1">{targetPoints} PT</p>
                    </div>
                    <div className="text-center">
                        <p className="text-[10px] font-black uppercase text-fuchsia-400 mb-2">Data Matrimonio</p>
                        <input type="datetime-local" value={weddingDate.substring(0, 16)} onChange={(e) => updateWeddingDate(e.target.value)} className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white" />
                    </div>
                </div>

                {/* TASTO CARICAMENTO FOTO DI GRUPPO */}
                <div className="pt-4 border-t border-white/10 text-center">
                    <p className="text-[10px] font-black uppercase text-yellow-400 mb-3 tracking-[0.2em]">Foto di Gruppo per il Diploma 📸</p>
                    <label className="block w-full bg-purple-600 text-white text-center py-4 rounded-2xl font-black cursor-pointer uppercase text-xs hover:bg-fuchsia-600 transition-all">
                        {diplomaPhoto ? "Aggiorna Foto di Gruppo" : "Carica Foto di Gruppo"}
          ...
          <div className="w-full bg-black/50 h-5 rounded-full overflow-hidden p-1 border border-white/20">
            <div className={`h-full rounded-full transition-all duration-1000 ${targetReached ? 'bg-gradient-to-r from-yellow-400 to-green-400' : 'bg-fuchsia-600'}`} style={{ width: `${Math.min((currentPoints / targetPoints) * 100, 100)}%` }}></div>
          </div>
        </div>

        {/* MESSAGGI */}
        {!isSettingsOpen && targetReached && (
          <div className="bg-fuchsia-600 text-white p-8 rounded-[2.5rem] text-center border-4 border-white mb-8 shadow-2xl animate-pulse">
            <h2 className="text-3xl font-black uppercase italic">🎓 DIPLOMA DISPONIBILE!</h2>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-2 mb-10">
          ...
          <button onClick={() => window.print()} className={`px-5 py-3 rounded-2xl text-[10px] font-black uppercase ml-auto shadow-lg transition-all active:scale-95 ${targetReached ? 'bg-yellow-400 text-black' : 'bg-green-600 text-white'}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {filteredChallenges.map((c) => (
            <div key={c.id} onClick={() => { setSelectedChallenge(c); setIsBonusSelected(c.bonus_achieved); }} className={`p-6 rounded-[2rem] border-2 transition-all active:scale-95 cursor-pointer ${c.is_completed ? 'bg-green-500/10 border-green-500/30' : 'bg-white/5 border-white/10'}`}>
              <div className="flex justify-between font-black text-yellow-400 mb-2"><span>+{c.points} PT</span> {c.is_completed && "✅"}</div>
              <h2 className="text-xl font-bold uppercase truncate">{c.title}</h2>
            </div>
          ))}
        </div>
        ...
        {selectedChallenge && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="bg-[#1a0521] border border-white/20 w-full max-w-lg rounded-[2.5rem] p-8 relative z-10 shadow-2xl overflow-y-auto max-h-[90vh]">
              <button onClick={() => setSelectedChallenge(null)} className="absolute top-6 right-6 text-xl">✕</button>
              ...
              {selectedChallenge.is_completed ? (
                <div className="space-y-4">
                  {isVideo(selectedChallenge.media_url) ? (
                    <video src={selectedChallenge.media_url} controls playsInline webkit-playsinline="true" className="w-full rounded-2xl border border-white/10" />
                  ) : (
                    <img src={selectedChallenge.media_url} className="w-full rounded-2xl border border-white/10" alt="Prova" />
                  )}
                ...
                <div className="space-y-6">
                  <input type="text" placeholder="Didascalia..." className="w-full bg-black/40 border border-white/10 p-5 rounded-2xl text-white outline-none focus:border-fuchsia-500" onChange={(e) => setCaption(e.target.value)} value={caption} />
                  ...
                  <label className="block w-full bg-white text-black text-center py-5 rounded-2xl font-black cursor-pointer uppercase hover:bg-yellow-400 transition-all active:scale-95 shadow-xl">📸 Carica Prova
