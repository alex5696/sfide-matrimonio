...
import { supabase } from "@/lib/supabase"; // Usiamo @ per evitare errori di percorso
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    async function fetchSettings() {
      const { data } = await supabase.from("Settings").select("wedding_date").eq('id', 1).maybeSingle();
    ...
    <main className="min-h-screen bg-gradient-to-br from-[#8000ff] via-[#b300ff] to-[#ff0080] flex flex-col items-center justify-center p-6 text-white text-center">
      ...
      <div className="grid grid-cols-4 gap-3 mb-12 max-w-sm w-full min-h-[80px]">
        {isMounted ? (
          ...
          <div className="col-span-4 text-xs font-bold uppercase opacity-40 animate-pulse text-center">Caricamento...</div>
        )}
      </div>
